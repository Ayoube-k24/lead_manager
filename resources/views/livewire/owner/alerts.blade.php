...
new class extends Component {
    ...
    public function resetAlertForm(): void
    {
        ...
        // Get first available type for owner
    ...
    public function getAvailableTypes(): array
    {
        ...
        $types = $service->getAvailableTypesForRole('call_center_owner');
    ...
    public function getTypeConditions(string $type): array
    {
        ...
        $roleAlertType = \App\Models\RoleAlertType::forRole('call_center_owner')
        ...
        if ($roleAlertType && $roleAlertType->default_conditions) {
            ...
            $conditions = ['status_slug' => null, 'agent_id' => null, 'call_center_id' => Auth::user()->call_center_id];
        }
        
        // For owner, automatically set their call center
    ...
    {
        // Security check: user can only modify alerts for call_center_owner role
        if ($alert->role_slug !== 'call_center_owner') {
    ...
    {
        // Security check: user can only delete alerts for call_center_owner role
        if ($alert->role_slug !== 'call_center_owner') {
    ...
    {
        // Filter alerts by call_center_owner role only
        $alerts = Alert::where('role_slug', 'call_center_owner')
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Configurez des alertes pour être notifié des événements importants de votre centre d\'appels') }}</p>
        </div>
    ...
    @if ($showGuide)
        <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
            <div class="border-b border-neutral-200 p-6 dark:border-neutral-700">
                ...
                <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Types d\'alertes disponibles pour les propriétaires de centres d\'appels') }}</p>
            </div>
...
<div>
            ...
            <flux:field>
                ...
                <flux:input wire:model="alertName" placeholder="{{ __('Ex: Pas de réponse - Seuil 10') }}" />
            ...
            @if ($alertType === 'status_threshold')
                ...
                <flux:field>
                    ...
                    <flux:select wire:model="alertConditions.agent_id">
                        <option value="">{{ __('Tous les agents de mon centre') }}</option>
                    ...
                    <flux:description>{{ __('Limiter l\'alerte aux leads d\'un agent spécifique de votre centre') }}</flux:description>
                </flux:field>
            @endif

            <flux:field>
                <flux:label>{{ __('Seuil') }}</flux:label>
                <flux:input wire:model="alertThreshold" type="number" step="0.01" min="0" placeholder="{{ __('Ex: 10') }}" />
