...
new class extends Component {
    ...
    public function resetAlertForm(): void
    {
        ...
        // Get first available type for agent
    ...
    public function getAvailableTypes(): array
    {
        ...
        $types = $service->getAvailableTypesForRole('agent');
    ...
    public function getTypeConditions(string $type): array
    {
        ...
        $roleAlertType = \App\Models\RoleAlertType::forRole('agent')
        ...
        if ($roleAlertType && $roleAlertType->default_conditions) {
            ...
            $conditions = ['status_slug' => null];
        }
        
        // For agents, automatically set their own ID and call center
    ...
    public function createAlert(): void
    {
        $rules = [
            ...
            'alertThreshold' => ['required', 'numeric', 'min:0'],
        ...
        // Automatically set agent_id and call_center_id for agents
    ...
    {
        // Security check: user can only modify alerts for agent role
        if ($alert->role_slug !== 'agent') {
            session()->flash('error', __('Vous n\'avez pas la permission de modifier cette alerte.'));
            return;
        }

        // Additional check: agent can only modify their own alerts
    ...
    {
        // Security check: user can only delete alerts for agent role
        if ($alert->role_slug !== 'agent') {
            session()->flash('error', __('Vous n\'avez pas la permission de supprimer cette alerte.'));
            return;
        }

        // Additional check: agent can only delete their own alerts
        if ($alert->user_id !== Auth::id()) {
            session()->flash('error', __('Vous n\'avez pas la permission de supprimer cette alerte.'));
            return;
        }
    ...
    {
        // Filter alerts by agent role and current user
        $alerts = Alert::where('role_slug', 'agent')
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">{{ __('Mes Alertes') }}</h1>
            <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Configurez des alertes pour être notifié des événements importants concernant vos leads') }}</p>
        </div>
    ...
    @if ($showGuide)
        <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
            <div class="border-b border-neutral-200 p-6 dark:border-neutral-700">
                ...
                <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Types d\'alertes disponibles pour les agents') }}</p>
            </div>
...
<div>
                ...
                <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                    {{ __('Configurez une alerte pour être notifié automatiquement sur vos propres leads') }}
                </p>
            </div>

            <flux:field>
                <flux:label>{{ __('Nom de l\'alerte') }}</flux:label>
                <flux:input wire:model="alertName" placeholder="{{ __('Ex: Leads en attente - Seuil 5') }}" />
            ...
            @if ($alertType === 'status_threshold')
                <flux:field>
                    ...
                    <flux:description>{{ __('Le statut pour lequel vous souhaitez être alerté sur vos propres leads') }}</flux:description>
                </flux:field>
            @endif

            <flux:field>
                <flux:label>{{ __('Seuil') }}</flux:label>
                <flux:input wire:model="alertThreshold" type="number" step="0.01" min="0" placeholder="{{ __('Ex: 5') }}" />
                <flux:error name="alertThreshold" />
                <flux:description>
                    {{ __('Nombre minimum de vos leads avec ce statut pour déclencher l\'alerte') }}
                </flux:description>
            </flux:field>
