...
new class extends Component {
    ...
    public function selectAll(): void
    {
        ...
        // Obtenir les IDs de la page actuelle depuis la pagination (même requête que dans with())
    ...
    public function delete(LeadStatus $status): void
    {
        try {
            ...
            $service->deleteStatus($status, $replacementStatus);
    ...
    public function confirmBulkDelete(): void
    {
        ...
        // Check if any selected status is used globally
    ...
    public function with(): array
    {
        ...
        $statusToDelete = $this->statusToDelete ? LeadStatus::withCount('leads')->find($this->statusToDelete) : null;
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Gérez les statuts pour organiser vos leads') }}</p>
        </div>
        <flux:button href="{{ route('admin.statuses.create') }}" variant="primary" icon="plus" wire:navigate>
            {{ __('Nouveau statut') }}
        </flux:button>
    </div>
    ...
    <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
        ...
        @else
            <div class="p-12 text-center">
                ...
                <flux:button href="{{ route('admin.statuses.create') }}" variant="primary" class="mt-4" wire:navigate>
                ...
                @if ($statusToDelete->leads_count > 0)
                    <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
                        <p class="text-sm font-medium text-yellow-900 dark:text-yellow-100">
                            {{ __('Attention : Ce statut est utilisé sur :count lead(s).', ['count' => $statusToDelete->leads_count]) }}
                        </p>
                        <p class="mt-1 text-sm text-yellow-800 dark:text-yellow-200">
                            {{ __('Veuillez sélectionner un statut de remplacement pour ces leads.') }}
                        </p>
                    </div>
                    ...
                    <flux:field>
                        ...
                        <flux:description>{{ __('Tous les leads avec ce statut seront mis à jour avec le statut de remplacement.') }}</flux:description>
                    </flux:field>
                @else
                    <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
                        <p class="text-sm text-neutral-600 dark:text-neutral-400">
                            {{ __('Ce statut n\'est utilisé sur aucun lead. La suppression est définitive.') }}
                        </p>
                    </div>
                @endif
...
<div>
            ...
            @if ($totalLeads > 0)
                <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
                    <p class="text-sm font-medium text-yellow-900 dark:text-yellow-100">
                        {{ __('Attention : Certains statuts sont utilisés sur :count lead(s) au total.', ['count' => $totalLeads]) }}
                    </p>
                    <p class="mt-1 text-sm text-yellow-800 dark:text-yellow-200">
                        {{ __('Veuillez sélectionner un statut de remplacement pour ces leads.') }}
                    </p>
                ...
                <flux:field>
                    ...
                    <flux:description>{{ __('Tous les leads avec les statuts sélectionnés seront mis à jour avec le statut de remplacement.') }}</flux:description>
                </flux:field>
            @else
                <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
                    <p class="text-sm text-neutral-600 dark:text-neutral-400">
                        {{ __('Les statuts sélectionnés ne sont utilisés sur aucun lead. La suppression est définitive.') }}
                    </p>
                </div>
            @endif
