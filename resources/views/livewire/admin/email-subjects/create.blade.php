...
use App\Http\Requests\StoreEmailSubjectRequest;
...
new class extends Component {
    ...
    {
        $validated = $this->validate((new StoreEmailSubjectRequest)->rules());

        EmailSubject::create($validated);

        session()->flash('message', __('Sujet d\'email créé avec succès !'));
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div>
        <h1 class="text-2xl font-bold">{{ __('Créer un sujet d\'email') }}</h1>
        <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Créez un nouveau sujet d\'email prédéfini pour les agents') }}</p>
    </div>
    ...
    <form wire:submit="store" class="space-y-6">
        ...
        <div class="flex items-center justify-between border-t border-neutral-200 pt-6 dark:border-neutral-700">
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove wire:target="store">{{ __('Créer le sujet') }}</span>
