...
new class extends Component {
    ...
    public function resetAlertForm(): void
    {
        ...
        // Get first available type for super_admin
        $availableTypes = $this->getAvailableTypes();
        $this->alertType = ! empty($availableTypes) ? array_key_first($availableTypes) : 'lead_stale';
    ...
    public function getAvailableTypes(): array
    {
        ...
        $types = $service->getAvailableTypesForRole('super_admin');
    ...
    public function getTypeConditions(string $type): array
    {
        ...
        $roleAlertType = \App\Models\RoleAlertType::forRole('super_admin')
    ...
    {
        // Security check: user can only modify alerts for super_admin role
        if ($alert->role_slug !== 'super_admin') {
    ...
    {
        // Security check: user can only delete alerts for super_admin role
        if ($alert->role_slug !== 'super_admin') {
    ...
    {
        // Filter alerts by super_admin role only
        $alerts = Alert::where('role_slug', 'super_admin')
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">{{ __('Alertes Super Admin') }}</h1>
            <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Configurez des alertes pour être notifié des événements importants au niveau système') }}</p>
        </div>
    ...
    @if ($showGuide)
        <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
            <div class="border-b border-neutral-200 p-6 dark:border-neutral-700">
                <h2 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{{ __('Guide des alertes Super Admin') }}</h2>
                <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{{ __('Types d\'alertes disponibles pour les Super Administrateurs') }}</p>
            </div>
...
<div>
            ...
            @if ($alertType === 'form_performance')
                <flux:field>
                    <flux:label>{{ __('Formulaire') }}</flux:label>
                    <flux:select wire:model="alertConditions.form_id">
                        <option value="">{{ __('Sélectionner un formulaire') }}</option>
                        @foreach (\App\Models\Form::all() as $form)
                            <option value="{{ $form->id }}">{{ $form->name }}</option>
                        @endforeach
                    </flux:select>
                    <flux:error name="alertConditions.form_id" />
                </flux:field>
            @endif

            @if ($alertType === 'lead_stale' || $alertType === 'high_volume' || $alertType === 'low_volume')
                <flux:field>
                    <flux:label>{{ __('Période (heures)') }}</flux:label>
                    <flux:input wire:model="alertConditions.hours" type="number" min="1" placeholder="{{ __('Ex: 24') }}" />
                    <flux:error name="alertConditions.hours" />
                </flux:field>
            @endif
            ...
            <flux:field>
                ...
                <flux:description>
                    ...
                    @elseif (in_array($alertType, ['agent_performance', 'conversion_rate', 'form_performance']))
